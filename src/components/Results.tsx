            ...
            <div className="section__content-gallery">
                {results.map((result: TResult)=> (
                    <div key={result.id} className={`section__gallery-item ${styles["results__gallery-item"]}`}>
                        <div className={`${styles["results__column-unite"]} section__gallery-title`}>Дело {`${result.case}`}</div>
                        ...
                        <div className={`${styles["results__column-unite"]} ${styles["results__result-comment"]}`}>Постановление Арбитражного суда Северо-Западного округа от 16.09.2024</div>
                    </div>
                ))}
            </div>
        </section>
