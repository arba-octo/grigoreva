                ...
                <div className="section__content-gallery">
                    {results.map((result: TResult) => (
                        <div key={result.id} className={`section__gallery-item ${styles["results__gallery-item"]}`}>
                            ...
                            <div className={styles["results__result-value"]}>{result.sumFirst.toLocaleString('ru-RU')} ₽</div>
                            <div>Сумма возмещения по нашей оценке:</div>
                            <div className={styles["results__result-value"]}>{result.sumNew.toLocaleString('ru-RU')} ₽</div>
                            <div>Результат: взыскали в пользу доверителя:</div>
                            <div className={styles["results__result-value"]}>{result.sumResult.toLocaleString('ru-RU')} ₽</div>
                            <div
